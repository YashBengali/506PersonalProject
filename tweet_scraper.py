...
def scrape(key, secret):
    ...
    csvFile = open('eth_tweets.csv', 'a')
    csvWriter = csv.writer(csvFile)
    for tweet in tp.Cursor(api.search_tweets, q='ethereum').items(500000):
        csvWriter.writerow([tweet.created_at,tweet.user.followers_count,tweet.favorite_count,tweet.text.encode('utf-8')])
    csvFile.close()
    print('[*] All Done')

def removeDups():
    csvFile = open("eth_tweets.csv", "r")
    lines = csvFile.read().split("\n")
    csvFile.close()
    
    noDupsCSV = open("eth_tweets_unique.csv", "w")
    for line in set(lines):
        noDupsCSV.write(line + "\n")
    noDupsCSV.close()

# def scrape():
#     csvFile = open('eth_tweets.csv', 'w')
#     csvWriter = csv.writer(csvFile)
#     csvWriter.writerow(['USERNAME','LINK TO TWEET', 'IS RETWEET', 'TIME', 'TEXT', 'LIKES', 'RETWEETS'])
#     from twitter_scraper import get_tweets
#     for tweet in get_tweets('ethereum', pages=30):
...
if __name__ == "__main__":
    ...
    # scrape(creds['api_key'], creds['api_secret_key'])
    removeDups()